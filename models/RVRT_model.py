...
class CharbonnierLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return loss
def validate_RVRT_model(args,model,dataset_val,valnoisestd,device,role):
    with torch.no_grad():
        window_size = model.window_size
        N, T, C, H, W = dataset_val.shape
        d_pad = (T // window_size[0] + 1) * window_size[0] - T
        dataset = torch.cat([dataset_val, torch.flip(dataset_val[:, -d_pad:, ...], [1])], 1)
        noise_level = torch.ones((1, 1, 1, 1, 1), device=device) * valnoisestd
        noise = torch.normal(mean=0, std=noise_level.expand_as(dataset))
        imgs_lq = dataset + noise
        noise_channel = noise_level.expand(N, T, 1, H, W)
        imgs_lq = torch.cat([imgs_lq, noise_channel], dim=2)
        output_seq = test_clip(imgs_lq,model)
        output = output_seq[:, :T, :, :, :]
        output = output.squeeze(0)
        dataset_val = dataset_val.squeeze(0)
        psnr = batch_psnr(output, dataset_val, data_range=1.0)
        return psnr
def test_clip(imgs_lq,model,context = torch.no_grad(),window_size = [2,8,8]):
    window_size = window_size
    ...
    with context:
        ...
        for h_idx in h_idx_list:
            for w_idx in w_idx_list:
                in_patch = imgs_lq[..., h_idx:h_idx + size_patch_testing, w_idx:w_idx + size_patch_testing]
                ...
                E[..., h_idx*1:(h_idx + size_patch_testing) * 1, w_idx * 1:(w_idx + size_patch_testing) * 1].add_(
                    out_patch)
                W[..., h_idx * 1:(h_idx + size_patch_testing) * 1, w_idx * 1:(w_idx + size_patch_testing) * 1].add_(
                    out_patch_mask)
        output = E.div_(W)
    return output
